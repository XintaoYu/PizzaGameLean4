...
def set_weight (cut : cuts n) (s : Finset (Fin (n + 1))) : ℝ :=
  ∑ x in s, cut.val x

def interval_weight (cut : cuts n) (a b : Fin (n + 1)) : ℝ :=
  set_weight cut (Fin.cIcc a b)

def red_weight (cut : cuts n) (a b : Fin (n + 1)) : ℝ :=
  set_weight cut (red_pieces a b)

def green_weight (cut : cuts n) (a b : Fin (n + 1)) : ℝ :=
  set_weight cut (green_pieces a b)

def semicircle_size (n : ℕ) : ℕ :=
  (n + 2) / 2

def is_semicircle (a b : Fin (n + 1)) : Prop :=
...
def good_start (cut : cuts n) (start : Fin (n + 1)) : Prop :=
  ∀ finish : Fin (n + 1),
    is_semicircle (n := n) start finish →
    red_weight cut start finish ≥ (1 : ℝ) / 3

def fB_strategy (_ : Odd (n + 1)) (start : Fin (n + 1)) : strategies n :=
  fun cut turn i =>
    if h0 : i = 0 then
      turn.val i = start
    else if Even i.val then
      ∃ a b : Fin (n + 1),
        current_state turn i = Fin.cIcc a b ∧
        turn.val i ∈ red_pieces a b ∧
        turn.val i ∈ Fin.cIcc start (fB_finish (n := n) start)
    else
      True

def strategy_guarantees (cut : cuts n) (s : strategies n) (bound : ℝ) : Prop :=
  ∀ game : games n, game.cut = cut → game.strategy = s → game.result ≥ bound

theorem set_weight_union_of_disjoint
    (cut : cuts n) (s t : Finset (Fin (n + 1)))
    (h_disj : Disjoint s t) :
    set_weight cut (s ∪ t) = set_weight cut s + set_weight cut t := by
  unfold set_weight
  rw [Finset.sum_union h_disj]

theorem red_green_disjoint
    {a b : Fin (n + 1)} :
    Disjoint (red_pieces a b) (green_pieces a b) := by
  refine Finset.disjoint_left.mpr ?_
  intro x hxR hxG
  rw [mem_red_pieces] at hxR
  rw [mem_green_pieces] at hxG
  have hne : ¬ Even (x - a).val := Nat.not_even_iff_odd.mpr hxG.right
  exact hne hxR.right

theorem red_green_union
    {a b : Fin (n + 1)} :
    red_pieces a b ∪ green_pieces a b = Fin.cIcc a b := by
  ext x
  constructor
  · intro hx
    rw [Finset.mem_union] at hx
    rcases hx with hxR | hxG
    · rw [mem_red_pieces] at hxR
      exact hxR.left
    · rw [mem_green_pieces] at hxG
      exact hxG.left
  · intro hx
    rw [Finset.mem_union]
    by_cases hEven : Even (x - a).val
    · left
      rw [mem_red_pieces]
      exact ⟨hx, hEven⟩
    · right
      rw [mem_green_pieces]
      exact ⟨hx, Nat.not_even_iff_odd.mp hEven⟩

theorem red_green_partition
    (a b : Fin (n + 1)) :
    Disjoint (red_pieces a b) (green_pieces a b) ∧
    red_pieces a b ∪ green_pieces a b = Fin.cIcc a b := by
  exact ⟨red_green_disjoint (a := a) (b := b), red_green_union (a := a) (b := b)⟩

theorem red_green_weight_sum
    (cut : cuts n) (a b : Fin (n + 1)) :
    red_weight cut a b + green_weight cut a b = interval_weight cut a b := by
  rcases red_green_partition (a := a) (b := b) with ⟨h_disj, h_union⟩
  unfold red_weight green_weight interval_weight
  rw [← h_union]
  symm
  exact set_weight_union_of_disjoint cut (red_pieces a b) (green_pieces a b) h_disj

theorem set_weight_nonneg
    (cut : cuts n) (s : Finset (Fin (n + 1))) :
    0 ≤ set_weight cut s := by
  unfold set_weight
  exact Finset.sum_nonneg (fun x _ => cut.property.1 x)

theorem interval_weight_ge_red_weight
    (cut : cuts n) (a b : Fin (n + 1)) :
    red_weight cut a b ≤ interval_weight cut a b := by
  have hsum := red_green_weight_sum cut a b
  have hnonneg : 0 ≤ green_weight cut a b :=
    set_weight_nonneg cut (green_pieces a b)
  linarith

theorem red_pieces_subset_cIcc
    {a b : Fin (n + 1)} :
    red_pieces a b ⊆ Fin.cIcc a b := by
  intro x hx
  exact (mem_red_pieces x).mp hx |>.left

theorem fB_first_move_mem
    (h : Odd (n + 1)) (cut : cuts n) (start : Fin (n + 1))
    (game : games n) :
    game.cut = cut →
    game.strategy = fB_strategy (n := n) h start →
    start ∈ alice's_pieces game := by
  intro hcut hstr
  rw [alice's_pieces_def]
  rw [Set.mem_toFinset, Set.mem_setOf]
  refine ⟨0, by simp, ?_⟩
...
theorem fB_even_move_is_red
    (h : Odd (n + 1)) (start : Fin (n + 1))
    (game : games n) (i : Fin (n + 1)) :
    game.strategy = fB_strategy (n := n) h start →
    i ≠ 0 →
    Even i.val →
    ∃ a b : Fin (n + 1),
      current_state game.turn i = Fin.cIcc a b ∧
      game.turn.val i ∈ red_pieces a b := by
  intro hstr hi0 hEven
  have hturn : game.strategy game.cut game.turn i := game.good_turn i
  rw [hstr] at hturn
  simp [fB_strategy, hi0, hEven] at hturn
  rcases hturn with ⟨a, b, hstate, hred, _harc⟩
  exact ⟨a, b, hstate, hred⟩

theorem fB_even_move_mem_fixed_arc
    (h : Odd (n + 1)) (start : Fin (n + 1))
    (game : games n) (i : Fin (n + 1)) :
    game.strategy = fB_strategy (n := n) h start →
    i ≠ 0 →
    Even i.val →
    game.turn.val i ∈ Fin.cIcc start (fB_finish (n := n) start) := by
  intro hstr hi0 hEven
  have hturn : game.strategy game.cut game.turn i := game.good_turn i
  rw [hstr] at hturn
  simp [fB_strategy, hi0, hEven] at hturn
  rcases hturn with ⟨a, b, _hstate, _hred, harc⟩
  exact harc

theorem fB_state_is_interval
    (game : games n) (i : Fin (n + 1)) :
    i ≠ 0 →
    ∃ a b : Fin (n + 1),
      current_state game.turn i = Fin.cIcc a b := by
  intro hi0
  exact current_state_eq_cIcc game i hi0

theorem current_state_ne_univ_of_ne_zero
    (game : games n) (i : Fin (n + 1)) :
    i ≠ 0 → current_state game.turn i ≠ Finset.univ := by
  intro hi0 hEq
  have hmemTaken : game.turn.val (i - 1) ∈ taken_pieces game.turn (i - 1) := by
    unfold taken_pieces
    rw [Finset.mem_image]
    exact ⟨i - 1, by simp [Finset.mem_Iic], rfl⟩
  have hmemState : game.turn.val (i - 1) ∈ current_state game.turn i := by
    rw [hEq]
    exact Finset.mem_univ _
  rw [current_state_of_ne_zero hi0, Finset.mem_sdiff] at hmemState
  exact hmemState.2 hmemTaken

theorem boundary_of_current_state_eq_endpoints
    (game : games n) (i : Fin (n + 1))
    (hi0 : i ≠ 0)
    {a b : Fin (n + 1)}
    (hstate : current_state game.turn i = Fin.cIcc a b) :
    boundary (current_state game.turn i) = {a, b} := by
  rw [hstate]
  apply cIcc_boundary
  intro huniv
  exact current_state_ne_univ_of_ne_zero game i hi0 (hstate.trans huniv)

theorem current_state_card
    (game : games n) (i : Fin (n + 1)) :
    i ≠ 0 → (current_state game.turn i).card = (n + 1) - i.val := by
  intro hi0
...
theorem endpoints_mem_red_of_odd_state
    {a b : Fin (n + 1)}
    (hodd : Odd (Fin.cIcc a b).card) :
    a ∈ red_pieces a b ∧ b ∈ red_pieces a b := by
  have hlen : Odd ((b - a).val + 1) := by
    simpa [Fin.cIcc_card_eq] using hodd
  constructor
  · rw [mem_red_pieces]
    constructor
    · exact Fin.left_mem_cIcc
    · simpa
  · have hsame := red_pieces_same_of_odd_length (a := a) (b := b) hlen
    rw [hsame, mem_red_pieces₀]
    constructor
    · exact Fin.right_mem_cIcc
    · simpa

theorem even_move_is_endpoint_of_state
    (h : Odd (n + 1)) (start : Fin (n + 1))
    (game : games n) (i : Fin (n + 1)) :
    game.strategy = fB_strategy (n := n) h start →
    i ≠ 0 →
    ...
    ∃ a b : Fin (n + 1),
      current_state game.turn i = Fin.cIcc a b ∧
      (game.turn.val i = a ∨ game.turn.val i = b) := by
  intro hstr hi0 hiEven
  rcases fB_even_move_is_red (n := n) h start game i hstr hi0 hiEven with ⟨a, b, hstate, _hred⟩
  have hlegal : game.turn.val i ∈ boundary (current_state game.turn i) := by
    exact (legal_turn.mp game.legality) i (Fin.pos_iff_ne_zero.mpr hi0)
  rw [boundary_of_current_state_eq_endpoints game i hi0 hstate, Finset.mem_insert, Finset.mem_singleton] at hlegal
  exact ⟨a, b, hstate, hlegal⟩

theorem even_move_is_red_endpoint_of_odd_state
    (h : Odd (n + 1)) (start : Fin (n + 1))
    (game : games n) (i : Fin (n + 1)) :
    game.strategy = fB_strategy (n := n) h start →
    i ≠ 0 →
    ...
    ∃ a b : Fin (n + 1),
      current_state game.turn i = Fin.cIcc a b ∧
      ((game.turn.val i = a ∧ a ∈ red_pieces a b) ∨ (game.turn.val i = b ∧ b ∈ red_pieces a b)) := by
  intro hstr hi0 hEven
  rcases even_move_is_endpoint_of_state (n := n) h start game i hstr hi0 hEven with ⟨a, b, hstate, hend⟩
  have hoddState : Odd (Fin.cIcc a b).card := by
    rw [← hstate]
    exact current_state_card_odd_of_even_round h game i hi0 hEven
  have hredEnds : a ∈ red_pieces a b ∧ b ∈ red_pieces a b :=
    endpoints_mem_red_of_odd_state hoddState
  refine ⟨a, b, hstate, ?_⟩
  rcases hend with rfl | rfl
  · exact Or.inl ⟨rfl, hredEnds.1⟩
  · exact Or.inr ⟨rfl, hredEnds.2⟩

theorem fB_alice_pieces_subset_fixed_arc
    (h : Odd (n + 1)) (cut : cuts n) (start : Fin (n + 1))
    (game : games n) :
    game.cut = cut →
    game.strategy = fB_strategy (n := n) h start →
    alice's_pieces game ⊆ Fin.cIcc start (fB_finish (n := n) start) := by
  intro hcut hstr x hx
  rw [alice's_pieces_def] at hx
  rw [Set.mem_toFinset, Set.mem_setOf] at hx
  rcases hx with ⟨i, hiEven, hxi⟩
  by_cases hi0 : i = 0
  · subst hi0
    rw [← hxi]
    exact Fin.left_mem_cIcc
...
def alice_rounds : Finset (Fin (n + 1)) :=
  Finset.univ.filter (fun i : Fin (n + 1) => Even i.val)

def bob_rounds : Finset (Fin (n + 1)) :=
  Finset.univ.filter (fun i : Fin (n + 1) => Odd i.val)

theorem mem_alice_rounds {i : Fin (n + 1)} :
    i ∈ alice_rounds (n := n) ↔ Even i.val := by
  simp [alice_rounds]

theorem mem_bob_rounds {i : Fin (n + 1)} :
    i ∈ bob_rounds (n := n) ↔ Odd i.val := by
  simp [bob_rounds]

theorem alice_pieces_eq_image_alice_rounds (game : games n) :
    alice's_pieces game = (alice_rounds (n := n)).image game.turn.val := by
  ext x
  constructor
  · intro hx
    rw [alice's_pieces_def] at hx
    rw [Set.mem_toFinset, Set.mem_setOf] at hx
    rcases hx with ⟨i, hiEven, hix⟩
    rw [Finset.mem_image]
    refine ⟨i, ?_, hix⟩
    rw [mem_alice_rounds]
    exact hiEven
  · intro hx
    ...
    rcases hx with ⟨i, hi_mem, hix⟩
    rw [alice's_pieces_def]
    rw [Set.mem_toFinset, Set.mem_setOf]
    refine ⟨i, ?_, hix⟩
    rw [mem_alice_rounds] at hi_mem
    exact hi_mem

theorem bob_pieces_eq_image_bob_rounds (game : games n) :
    bob's_pieces game = (bob_rounds (n := n)).image game.turn.val := by
  ext x
  constructor
  · intro hx
    rw [bob's_pieces_def] at hx
    rw [Set.mem_toFinset, Set.mem_setOf] at hx
    rcases hx with ⟨i, hiOdd, hix⟩
    rw [Finset.mem_image]
    refine ⟨i, ?_, hix⟩
    rw [mem_bob_rounds]
    exact hiOdd
  · intro hx
    ...
    rw [Set.mem_toFinset, Set.mem_setOf]
    refine ⟨i, ?_, hix⟩
    rw [mem_bob_rounds] at hi_mem
    exact hi_mem

theorem alice_pieces_card_eq (game : games n) :
    (alice's_pieces game).card = (alice_rounds (n := n)).card := by
  rw [alice_pieces_eq_image_alice_rounds]
  exact Finset.card_image_of_injective _ game.turn.property

theorem bob_pieces_card_eq (game : games n) :
    (bob's_pieces game).card = (bob_rounds (n := n)).card := by
  rw [bob_pieces_eq_image_bob_rounds]
  exact Finset.card_image_of_injective _ game.turn.property

theorem set_weight_image_of_injective
    (cut : cuts n) (s : Finset (Fin (n + 1))) (f : Fin (n + 1) → Fin (n + 1))
    (hf : Function.Injective f) :
    set_weight cut (s.image f) = ∑ i in s, cut.val (f i) := by
  unfold set_weight
  rw [Finset.sum_image]
  · intro a _ b _ hab
    exact hf hab

theorem result_eq_weight_of_alice_rounds_image (game : games n) :
    game.result = set_weight game.cut ((alice_rounds (n := n)).image game.turn.val) := by
  unfold games.result
  have hsum :
      (∑ i with Even i.val, game.cut.val (game.turn.val i))
      =
      ∑ i in alice_rounds (n := n), game.cut.val (game.turn.val i) := by
    simp [alice_rounds]
  rw [hsum]
  symm
  exact set_weight_image_of_injective game.cut (alice_rounds (n := n)) game.turn.val game.turn.property

theorem result_eq_weight_of_alice_pieces (game : games n) :
    game.result = set_weight game.cut (alice's_pieces game) := by
  rw [alice_pieces_eq_image_alice_rounds]
  exact result_eq_weight_of_alice_rounds_image game

theorem fB_terminal_subset_arc
    (h : Odd (n + 1)) (cut : cuts n) (start : Fin (n + 1))
    (game : games n) :
    game.cut = cut →
    game.strategy = fB_strategy (n := n) h start →
...
theorem fB_terminal_card_match
    ...
    game.strategy = fB_strategy (n := n) h start →
    is_semicircle (n := n) start finish →
    alice's_pieces game ⊆ Fin.cIcc start finish →
    (alice's_pieces game).card = (Fin.cIcc start finish).card := by
  intro hcut hstr hsemi hsub
  have hAlice :
      (alice's_pieces game).card = (alice_rounds (n := n)).card :=
    alice_pieces_card_eq game
  have hArc :
      (Fin.cIcc start finish).card = (alice_rounds (n := n)).card := by
    rw [hsemi, alice_rounds_card_eq_semicircle_size]
  omega

theorem fB_terminal_arc
    (h : Odd (n + 1)) (cut : cuts n) (start : Fin (n + 1))
    (game : games n) :
    game.cut = cut →
    game.strategy = fB_strategy (n := n) h start →
    ∃ finish : Fin (n + 1),
      ...
      alice's_pieces game = Fin.cIcc start finish := by
  intro hcut hstr
  rcases fB_terminal_subset_arc (n := n) h cut start game hcut hstr with
    ⟨finish, hsemi, hsub⟩
  have hcard :
      (alice's_pieces game).card = (Fin.cIcc start finish).card :=
...
theorem result_eq_interval_weight_of_terminal_arc
    ...
    ∃ finish : Fin (n + 1),
      is_semicircle (n := n) start finish ∧
      game.result = interval_weight cut start finish := by
  intro hcut hstr
...
theorem one_third_from_good_start
    (h : Odd (n + 1)) (cut : cuts n) (start : Fin (n + 1)) :
    good_start cut start →
    strategy_guarantees cut (fB_strategy (n := n) h start) ((1 : ℝ) / 3) := by
  intro hgood game hcut hstr
  rcases result_eq_interval_weight_of_terminal_arc (n := n) h cut start game hcut hstr with
    ⟨finish, hsemi, hresult⟩
  rw [hresult]
  have hgood' : (1 : ℝ) / 3 ≤ red_weight cut start finish := hgood finish hsemi
...
theorem alice_rounds_disjoint_bob_rounds :
    Disjoint (alice_rounds (n := n)) (bob_rounds (n := n)) := by
  refine Finset.disjoint_left.mpr ?_
  intro x hxA hxB
  rw [mem_alice_rounds] at hxA
  rw [mem_bob_rounds] at hxB
  exact (Nat.not_even_iff_odd.mpr hxB) hxA
...
theorem even_odd_weight_sum
    (cut : cuts n) :
    (∑ i with Even i.val, cut.val i) + (∑ i with Odd i.val, cut.val i) = 1 := by
  have hsum :=
    set_weight_union_of_disjoint cut (alice_rounds (n := n)) (bob_rounds (n := n))
      (alice_rounds_disjoint_bob_rounds (n := n))
  rw [alice_rounds_union_bob_rounds (n := n)] at hsum
  unfold set_weight at hsum
  simp [alice_rounds, bob_rounds] at hsum
  linarith [cut.property.2]

theorem even_case_alice_subset_alice_rounds
    (h : Even (n + 1)) (game : games n)
    (hstr : game.strategy = even_case_strategy (n := n) h)
    (hpref : ∑ i with Even i.val, game.cut.val i ≥ ∑ i with Odd i.val, game.cut.val i) :
  ...
  have hturn : game.strategy game.cut game.turn i := game.good_turn i
  rw [hstr] at hturn
  simp [even_case_strategy, hiEven, hpref] at hturn
  rw [mem_alice_rounds, ← hxi]
  exact hturn

theorem even_case_alice_subset_bob_rounds
    (h : Even (n + 1)) (game : games n)
    (hstr : game.strategy = even_case_strategy (n := n) h)
    (hpref : ¬ ∑ i with Even i.val, game.cut.val i ≥ ∑ i with Odd i.val, game.cut.val i) :
    alice's_pieces game ⊆ bob_rounds (n := n) := by
  intro x hx
  rw [alice's_pieces_def] at hx
  rw [Set.mem_toFinset, Set.mem_setOf] at hx
  rcases hx with ⟨i, hiEven, hxi⟩
  ...
  rw [hstr] at hturn
  simp [even_case_strategy, hiEven, hpref] at hturn
  rw [mem_bob_rounds, ← hxi]
  exact hturn

theorem even_case_alice_eq_alice_rounds
    (h : Even (n + 1)) (game : games n)
    (hstr : game.strategy = even_case_strategy (n := n) h)
    (hpref : ∑ i with Even i.val, game.cut.val i ≥ ∑ i with Odd i.val, game.cut.val i) :
    alice's_pieces game = alice_rounds (n := n) := by
...
theorem even_case₀ (h : Even (n + 1)) : Nonempty (games_with_strategy (even_case_strategy h)) ∧ ∀ game : games_with_strategy (even_case_strategy h), game.val.result ≥ (1 : ℝ) / 2 := by
  ...
  · refine Nonempty.intro ?_
    let uniform_cut : cuts n := ⟨fun _ => (1 : ℝ) / (n + 1), by
        constructor
        · intro i
          positivity
        · show ∑ _, (1 : ℝ) / (n + 1) = 1
          ...
          field_simp⟩
    let id_turn : turns n := ⟨fun i => i, fun _ _ eq => eq⟩
    have hlegal : basic_rule id_turn := by
      rw [legal_turn]
      intro i hpos
      have hi0 : i ≠ 0 := Fin.pos_iff_ne_zero.mp hpos
      rw [boundary, Set.mem_toFinset, Set.mem_setOf]
      constructor
      ...
      · right
        rw [current_state_of_ne_zero hi0, Finset.mem_sdiff]
        intro hmem
        have hTaken : i - 1 ∈ taken_pieces id_turn (i - 1) := by
          rw [taken_pieces, Finset.mem_image]
          exact ⟨i - 1, by simp [Finset.mem_Iic], rfl⟩
        exact hmem.2 hTaken
    have hpref :
        ∑ i with Even i.val, uniform_cut.val i ≥
          ∑ i with Odd i.val, uniform_cut.val i := by
      have hEvenSum :
          ∑ i with Even i.val, uniform_cut.val i =
            ((alice_rounds (n := n)).card : ℝ) * ((1 : ℝ) / (n + 1)) := by
        simp [uniform_cut, alice_rounds, nsmul_eq_mul]
      have hOddSum :
          ∑ i with Odd i.val, uniform_cut.val i =
            ((bob_rounds (n := n)).card : ℝ) * ((1 : ℝ) / (n + 1)) := by
        simp [uniform_cut, bob_rounds, nsmul_eq_mul]
      rw [hEvenSum, hOddSum, alice_rounds_card_eq_half_of_even (n := n) h,
  ...
  · intro game
    by_cases hpref : ∑ i with Even i.val, game.val.cut.val i ≥ ∑ i with Odd i.val, game.val.cut.val i
    · have hEq : alice's_pieces game.val = alice_rounds (n := n) :=
        even_case_alice_eq_alice_rounds (n := n) h game.val game.property hpref
      have hresult :
          game.val.result = ∑ i with Even i.val, game.val.cut.val i := by
        rw [result_eq_weight_of_alice_pieces, hEq]
        unfold set_weight
        simp [alice_rounds]
      rw [hresult]
      have htotal := even_odd_weight_sum (n := n) game.val.cut
      nlinarith
    · have hEq : alice's_pieces game.val = bob_rounds (n := n) :=
        even_case_alice_eq_bob_rounds (n := n) h game.val game.property hpref
      have hresult :
          game.val.result = ∑ i with Odd i.val, game.val.cut.val i := by
        rw [result_eq_weight_of_alice_pieces, hEq]
        unfold set_weight
        simp [bob_rounds]
      rw [hresult]
